...
class HomeController extends Controller
{
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header('Verimake');
            $content->description('后台文章管理系统');  

            $content->row(function ($row) {
                $row->column(6, new InfoBox('管理员', 'users', 'aqua', '/admin/auth/users', '1'));
                // $row->column(6, new InfoBox('New Orders', 'shopping-cart', 'green', '/admin/orders', '150%'));
                $row->column(6, new InfoBox('文章', 'book', 'yellow', '/admin/posts', $posts=Blog::count()));
                // $row->column(6, new InfoBox('Documents', 'file', 'red', '#', '698726'));
            });
